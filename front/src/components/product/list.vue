    ...
    <table>
      ...
      >
        ...
        <td class="space-x-1">
          ...
          <button
            @click="clickDeleteBtn(item.pr_id, index)"
...
export default Vue.extend({
  ...
  methods: {
    clickDeleteBtn(pr_id: string): void {
      this.$emit('deleteProduct', pr_id)
    },
    clickEditBtn(pr_id: string) {
      this.$emit('editProduct', pr_id)
    },
  },
})
</script>
